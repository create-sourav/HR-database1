...
print("Attrition rate:", ((df["Attrition"]=="Yes").sum() /len(df["Attrition"])*100),"%")   # Attrition_rate = 16.12%
...
encoders["Attrition"] = LabelEncoder()
y = encoders["Attrition"].fit_transform(df["Attrition"].astype(str))
...
for name, model in models.items():
    ...
    train_loss = log_loss(y_train_resample, y_train_prob)
...
X = x # Ensure X refers to the correctly one-hot encoded feature set
...
df["Predicted_Attrition"] = encoders["Attrition"].inverse_transform(
    RandomForest_tuned.predict(x_scaled)
)

df["Attrition_Probability"] = RandomForest_tuned.predict_proba(x_scaled)[:, 1]

plt.figure(figsize=(8,5))
sns.kdeplot(data=df, x="Attrition_Probability", hue="Attrition", fill=True)
